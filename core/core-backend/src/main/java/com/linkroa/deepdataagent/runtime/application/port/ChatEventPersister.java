 ...
 *       的完整性前置校验，分两段——① 事务<b>外</b>先 {@code flush()} 整队排空（逐条独立提交，
 *       他会话事件不进入本会话 JDBC 事务、不随本会话回滚丢失）；② 事务首行按会话
 *       {@code isPoisoned} 校验落库失败标记，命中即由调用方抛业务异常令外层事务整批回滚，
 *       杜绝「状态已迁移而事件表缺中间事件」的回放不一致。</li>
 * </ul>
 */
public interface ChatEventPersister {
     ...
     * 校验落库失败标记共同构成严格排空协议</b>：终态 / HITL 挂起须在开启事务<b>之前</b>调用本方法整队排空
     * （独立提交），再于事务首行按会话校验落库失败标记、命中即抛令事务回滚。MUST NOT 把排空放进状态迁移
     * 事务内做（他会话事件行会随本会话回滚一并丢失，构成跨会话事件表污染）。</p>
     */
    void flush();

    /**
     * 查询该会话是否存在「重试耗尽仍未落库」的事件（严格排空协议·事务首行校验落库失败标记）。
     * <p>落库失败标记仅在落库线程持锁处理队列条目的瞬间置位；调用方先行 {@link #flush()} 排空后，
     * 本查询结果即与排空结果一致（终态 / HITL 挂起事务首行专用，命中由调用方抛
     * {@link com.linkroa.deepdataagent.shared.exception.DeepDataAgentException} 令事务回滚、
     * 会话状态不迁移）。按 sessionId 隔离，不存在跨会话误判。</p>
     *
     * @param sessionId 待确认落库完整性的会话 ID
     * @return true=该会话存在重试耗尽仍未落库的事件（事件表不可信，须拒绝状态迁移）
     */
    boolean isPoisoned(String sessionId);

    /**
     * 清除该会话的持久化失败标志（新一轮启动 CAS 提交成功后调用）。
     * <p>幂等内存操作：旧轮丢失的事件属已回滚旧轮（事实已在结构化 ERROR 留痕），
     * 新一轮不继承旧轮故障，避免会话永久不可用。</p>
     *
