 ...
 * 记忆库引用由解析服务映射为 {@link MemoryStoreRef}，此处原样透传进装配规格。</p>
 */
...
public interface AgentAssemblyConvert {
     ...
     * @param sandbox         沙箱规格（由解析服务按版本引用环境格式化而来，未引用回退默认规格）
    ...
    @Mapping(target = "memoryStoreRefs", source = "memoryStoreRefs")
    AgentAssemblySpec toSpec(ResolvedAgentAssemblyDTO dto, AgentAssemblySpec.Sandbox sandbox,
