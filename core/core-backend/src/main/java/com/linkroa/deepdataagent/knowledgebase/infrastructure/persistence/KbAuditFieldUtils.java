...
/**
 * KB BC 实体审计字段显式填充工具类（包内私有，纯静态、防实例化）。
 * <p>KB 侧实体已脱离 {@code shared.BaseEntity} 与 {@code MybatisPlusMetaObjectHandler}
 * 体系（彻底物理删）：audit 字段无 {@code @TableField(fill=...)}
 * 注解，全局 handler 的 strictInsertFill 对它们完全不触发，必须在 Repository 层
 ...
 * <p>与 RAG BC 的 {@code RagAuditFieldUtils} 行为对称、互不引用（两 BC 各自持有）。</p>
 */
public final class KbAuditFieldUtils {
     ...
     */
    public static void fillInsert(KbAuditable entity, String operator) {
     ...
     * @param operator 操作人（null/空白回落 {@link #DEFAULT_OPERATOR}）
     */
    public static void fillUpdate(KbAuditable entity, String operator) {
        if (ObjectUtils.isEmpty(entity)) {
            return;
        }
        entity.setUpdatedAt(now());
