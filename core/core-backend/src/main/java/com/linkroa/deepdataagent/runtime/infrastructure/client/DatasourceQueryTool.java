 ...
 * 预览表数据。所有能力均为只读，凭证/连接细节由 {@link DatasourceApi} 收口，不跨 BC。</p>
 */
public class DatasourceQueryTool {

    private static final int DEFAULT_LIMIT = 10;

    private final DatasourceApi queryApi;
    private final List<Long> dataSourceIds;

    public DatasourceQueryTool(DatasourceApi queryApi, List<Long> dataSourceIds) {
    ...
    public String listDatasources() {
        List<DatasourceReferenceDTO> refs = queryApi.listDatasources(dataSourceIds);
        if (refs.isEmpty()) {
            return "当前无可用数据源";
        }
    ...
    public String listTables(@ToolParam(name = "dataSourceId", required = true, description = "数据源 ID") Long dataSourceId) {
        ...
        List<String> tables = queryApi.listTableNames(dataSourceId);
        if (tables.isEmpty()) {
            return "该数据源下无表";
        }
        return String.join("\n", tables);
    ...
    public String previewTable(
        ...
        requireAllowed(dataSourceId);
        DatasourcePreviewDTO preview = queryApi.previewTable(dataSourceId, tableName, limit == null ? DEFAULT_LIMIT : limit);
