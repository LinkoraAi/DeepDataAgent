...
/**
 * 聊天事件 payload 转换（应用层，承接原基础设施层 Adapter 的职责）。
 ...
 * 标记块结束。装配决策与应用层编排（持久化 / 广播 / span）解耦，本类为无状态纯转换器：
 * 状态累积（工具入参聚合、head+tail 截断窗口）由领域模型 {@code AgentRunState} 承载。</p>
 */
@Mapper
public interface ChatEventPayloadConvert {

    ChatEventPayloadConvert INSTANCE = Mappers.getMapper(ChatEventPayloadConvert.class);
    ...
    String TYPE_TOOL_CALL = "tool_call";
     ...
     * 推理增量（is_last=false）。
     */
    default AssembledEvent thinkingDelta(String blockId, String text) {
     ...
     */
    default AssembledEvent thinkingEnd(String blockId) {
        return contentEvent(ChatEventType.THINKING,
     ...
     */
    default AssembledEvent textDelta(String blockId, String text) {
     ...
     */
    default AssembledEvent textEnd(String blockId) {
        return contentEvent(ChatEventType.MESSAGE,
                block(TYPE_TEXT, "", blockId, null, null, Map.of()), true);
    }

    /**
     * 用户消息回显（发送方为用户，is_last=true 单块收尾）。
     */
    default AssembledEvent userMessage(String text) {
        return contentEvent(ChatEventType.USER_MESSAGE,
                block(TYPE_TEXT, text, null, null, null, Map.of()), true);
    }
     ...
     */
    default AssembledEvent toolCallStart(String toolCallId, String toolName) {
     ...
     */
    default AssembledEvent toolCallEnd(String toolCallId, String toolName, String argsJson) {
        return contentEvent(ChatEventType.TOOL_CALL,
     ...
     */
    default AssembledEvent toolResultDelta(String toolCallId, String toolName, String headText) {
     ...
     */
    default AssembledEvent toolResultEnd(String toolCallId, String toolName, String output, boolean truncated) {
        return contentEvent(ChatEventType.TOOL_CALL_OUTPUT,
                block(TYPE_TOOL_RESULT, null, null, toolCallId, toolName,
    ...
    private Map<String, Object> parseInput(String inputJson) {
        ...
        try {
            Map<String, Object> parsed = OBJECT_MAPPER.readValue(inputJson, MAP_TYPE);
