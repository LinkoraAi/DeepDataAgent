...
public class ApiConnectionStrategy implements DatasourceConnectionStrategy {
    ...
    public ConnectionTestResult testConnection(DatasourceConnection connection) {
        try {
            List<ApiSchema> schemas = apiSchemaRepository.findByConnectionId(connection.id());
            if (schemas.isEmpty()) {
                return ConnectionTestResult.fail("未找到API配置");
            }
            schemas.forEach(schema -> paginationHandler.executeOnce(schema, null, Map.of()));
    ...
    public ConnectionTestResult testConnection(ApiSchema apiSchema) {
        try {
            paginationHandler.executeOnce(apiSchema, null, Map.of());
            return ConnectionTestResult.ok();
        } catch (Exception e) {
            log.warn("API连接测试失败: {}", e.getMessage());
            return ConnectionTestResult.fail("API连接失败: " + e.getMessage());
    ...
    public List<Map<String, Object>> previewData(DatasourceConnection connection, String schemaName, String tableName, int limit) {
        try {
            ...
            PaginatedApiResult result = paginationHandler.executeOnce(apiSchema, null, Map.of());
