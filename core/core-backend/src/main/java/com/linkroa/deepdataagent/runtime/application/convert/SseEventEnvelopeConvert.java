...
/**
 * 领域聊天事件 → {@link SseEventEnvelope} 信封转换器（无状态纯装配）。
 * 将 {@link ChatEvent} 映射为对外 {@code Message} 信封：{@code type} 取领域事件类型小写、
 * {@code role} 由事件类型派生（工具类为 tool、用户回显为 user、其余为 assistant）、{@code content} 统一为
 * ContentBlock 数组——块事件（THINKING / MESSAGE / USER_MESSAGE / TOOL_CALL / TOOL_CALL_OUTPUT）直取
 ...
 * 额外将 {@code status} 更名为 {@code session_status}（值保持枚举名不变）。
 */
@Mapper
public interface SseEventEnvelopeConvert {
    ...
    /** 事件类型 → 信封 role：工具调用 / 工具结果归为 tool，用户消息回显归为 user，其余归为 assistant。 */
    private static String roleOf(ChatEventType type) {
        if (type == ChatEventType.TOOL_CALL || type == ChatEventType.TOOL_CALL_OUTPUT) {
            return "tool";
        }
        if (type == ChatEventType.USER_MESSAGE) {
            return "user";
        }
        return "assistant";
    }
