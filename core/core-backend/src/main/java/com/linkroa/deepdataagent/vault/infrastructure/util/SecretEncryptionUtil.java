...
/**
 * 凭证密钥加密工具（AES/GCM/NoPadding）。
 * <p>复用现有 AES/GCM 加密形态，但使用 vault BC 独立密钥
 * （{@code vault.encryption.key}），实现密钥（Secret）与数据源密码、模型凭证的密钥隔离。</p>
 */
@Component
public class SecretEncryptionUtil {
    ...
    public SecretEncryptionUtil(VaultEncryptionProperties encryptionProperties) {
    ...
    private byte[] resolveKey() throws Exception {
        ...
        if (key == null || key.isBlank()) {
            throw new IllegalStateException("未配置密钥加密密钥");
        }
