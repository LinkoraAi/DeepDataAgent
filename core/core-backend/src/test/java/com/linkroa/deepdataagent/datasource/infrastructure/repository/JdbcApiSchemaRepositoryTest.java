...
import com.linkroa.deepdataagent.datasource.domain.model.ApiRequestConfig;
...
class JdbcApiSchemaRepositoryTest extends DatasourceRepositoryTestSupport {
    ...
    void should_saveAndReturnSchema_when_save_given_validApiSchema() {
        ApiSchema apiSchema = new ApiSchema(null, 1L, "users-api", "/api/users", HttpMethod.GET, ApiRequestConfig.defaultConfig(), null, null, null, null);

        ApiSchema saved = repository.save(apiSchema);

        assertNotNull(saved.id());
        assertEquals("users-api", saved.name());
        assertEquals("/api/users", saved.url());
        assertEquals(HttpMethod.GET, saved.method());
    }

    @Test
    void should_findById_when_saved_given_id() {
        ApiSchema saved = repository.save(new ApiSchema(null, 1L, "test-api", "/api/test", HttpMethod.POST, ApiRequestConfig.defaultConfig(), null, null, null, null));
    ...
    void should_updateSchema_when_update_given_existingSchema() {
        ...
        ApiSchema updated = new ApiSchema(saved.id(), 1L, "new-name", "/new", HttpMethod.POST, ApiRequestConfig.defaultConfig(), saved.createdAt(), saved.updatedAt(), saved.createdBy(), saved.updatedBy());
        repository.update(updated);

        Optional<ApiSchema> found = repository.findById(saved.id());
        assertTrue(found.isPresent());
        assertEquals("new-name", found.get().name());
        assertEquals("/new", found.get().url());
    ...
    @Test
    void should_saveAndRetrieveConfig_when_save_given_apiRequestConfig() {
        ApiRequestConfig config = ApiRequestConfig.defaultConfig();
        ApiSchema apiSchema = new ApiSchema(null, 1L, "config-test", "/api/config", HttpMethod.GET, config, null, null, null, null);

        ApiSchema saved = repository.save(apiSchema);
        Optional<ApiSchema> found = repository.findById(saved.id());

        assertTrue(found.isPresent());
        assertNotNull(found.get().config());
        assertNotNull(found.get().url());
        assertEquals("/api/config", found.get().url());
    }
}
