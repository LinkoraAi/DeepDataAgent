...
class NL2SqlServiceTest {
    ...
    @BeforeEach
    void setUp() {
        service = new NL2SqlService(nl2SqlPort, sqlValidationPort, 3);
    }

    @Test
    void should_returnSql_when_convert_given_generationAndValidationSucceed() {
        // given
        when(nl2SqlPort.generateSql(any(), anyString(), anyString(), anyString(), any()))
        ...
        verify(nl2SqlPort, times(1))
                .generateSql(1L, "查询用户", "schema", "mysql", null);
    ...
    void should_returnSql_when_convert_given_firstAttemptFailsThenSucceeds() {
        ...
        // then
        assertEquals("SELECT * FROM t", result);
        verify(nl2SqlPort, times(2))
                .generateSql(any(), anyString(), anyString(), anyString(), any());
    }
    ...
    void should_passSessionIdAndRetryContext_when_convert_given_sessionIdAndFailure() {
        ...
        verify(nl2SqlPort, times(2))
                .generateSql(eq(1L), eq("查询用户"), contextCaptor.capture(), eq("mysql"), sessionCaptor.capture());
    ...
    void should_includeInvalidSqlInRetryContext_when_convert_given_validationFails() {
        ...
        ArgumentCaptor<String> contextCaptor = ArgumentCaptor.forClass(String.class);
        verify(nl2SqlPort, times(3))
                .generateSql(eq(1L), eq("查询用户"), contextCaptor.capture(), eq("mysql"), any());
        // 首次使用原始 schema
        assertEquals("schema", contextCaptor.getAllValues().get(0));
        // 后续每次重试都携带违规 SQL 与原因
